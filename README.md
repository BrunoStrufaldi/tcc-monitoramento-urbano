...
- Para inferência real: `pip install -r requirements-yolo.txt` e o peso local `ml/models/yolo11n.pt` (ou pesos urbanos próprios)
...
- Opcionalmente, o backend pode rodar monitoramento 100% automático — sem operador nem câmera obrigatória — ligado por `GX_MONITORAMENTO_ATIVO=true` (desligado por padrão, inclusive em testes). Dois módulos independentes:
  - **Trânsito** (`live_detection`, exige `backend/requirements-yolo.txt`): lê periodicamente o snapshot de câmeras públicas CET-SP (`https://cameras.cetsp.com.br/Cams/{id}/1.jpg`, atualizado por HTTP, não RTSP) — uma câmera avulsa via `GX_CAMERA_SNAPSHOT_URL`, e/ou todas as 11 câmeras já catalogadas (`cet_camera_catalog.py`) via `GX_TRANSITO_MONITORAR_CATALOGO=true`, cada uma em thread própria. O YOLO/COCO padrão só reconhece objetos (veículo, ônibus, caminhão, moto), não "trânsito" como classe — por isso o sistema conta quantos veículos aparecem juntos no mesmo frame e, a partir de `GX_TRANSITO_MIN_VEICULOS` (padrão 20), sinaliza um único evento de congestionamento, **sem confirmação humana**. Esse índice por contagem de veículos é corroborado, quando `TOMTOM_API_KEY` está configurada, com a velocidade atual x livre do trecho na TomTom Traffic API (dado ao vivo, não em lote) — as duas fontes entram como a mesma dimensão de clima no Data Fusion, com peso combinado quando concordam.
  - **Alagamento e acidente de trânsito** (`context_monitor` + `geosampa_source`, não depende de câmera): nenhum dos dois tem classe visual no YOLO padrão nem existe API pública de "aconteceu agora" para eles em SP (verificado por busca real, não presumido — inclusive Waze for Cities, que exige convênio formal da prefeitura com o Google, e o site do CGE, que não tem API, só HTML). O que existe são os datasets oficiais da Defesa Civil e da CET no GeoSampa (WFS público, camadas `risco_ocorrencia_alagamento` e `acidente_cet`, coordenada real, mas recarregados em lote — alagamento com defasagem de semanas a poucos meses, **acidente de trânsito com defasagem de anos** no momento em que isso foi verificado, então essa camada específica funciona como contexto histórico, não como monitoramento ao vivo). O sistema consulta esses datasets periodicamente, descarta o que já processou (tabela `ocorrencias_externas`, evita duplicar), e para cada ocorrência nova busca o clima atual naquele ponto exato — e, se houver uma câmera pública da CET conhecida por perto (`cet_camera_catalog.py`), roda o YOLO nela (confirmação direta se a classe bater com o modelo dedicado de incidentes, senão sinal indireto com confiança limitada). Tudo isso alimenta o Data Fusion (peso redistribuído entre as dimensões disponíveis). Só confirma (status `ativo` + cria `Notificacao`) quando a confiabilidade calculada passa de `GX_CONFIRMACAO_CONFIANCA_MINIMA` (padrão 0,75); abaixo disso, fica `em_analise`. (Queda de árvore existiu aqui como terceiro tipo monitorado e foi removida por decisão do grupo de reduzir o escopo do TCC.)
